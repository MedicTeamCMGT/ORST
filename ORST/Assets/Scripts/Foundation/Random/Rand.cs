...
namespace ORST.Foundation.Random {
    public static class Rand {
        private const float TWO_PI = 6.2831853071795864f;
        ...
        public static IRandomProvider RandomProvider {
            ...
            set {
                uint currentSeed = s_RandomProvider.Seed;
        ...
        /// <summary>
        /// Replaces the current seed with <paramref name="replacementSeed"/>.
        /// Use <see cref="PopState"/> to undo this operation and retrieve the original state
        /// </summary>
        ...
        public static void PopState() {
            if (s_StateStack.Count == 0) {
                UnityEngine.Debug.LogError("State stack is empty when trying to call PopState");
                return;
            }
        ...
        public static float Gaussian(float centerX = 0.0f, float widthFactor = 1f) {
            return Mathf.Sqrt(-2f * Mathf.Log(Float)) * Mathf.Sin(TWO_PI * Float) * widthFactor + centerX;
        }

        public static float GaussianAsymmetric(float centerX = 0.0f, float lowerWidthFactor = 1f, float upperWidthFactor = 1f) {
            float num = Mathf.Sqrt(-2f * Mathf.Log(Float)) * Mathf.Sin(TWO_PI * Float);
            if (num <= 0.0)
        ...
        private sealed class DefaultRandomProvider : IRandomProvider {
            ...
            public float GetFloat(uint iterations) {
                return (float) ((this.GetInt(iterations) - (double) int.MinValue) / uint.MaxValue);
            }

            private uint GetHash(int value) {
                uint num1 = Rotate(this.Seed + 409645513U + 4U + (uint) (value * -1028477379), 17) * 658815097U;
                uint num2 = (num1 ^ (num1 >> 15)) * 562027853U;
                uint num3 = (num2 ^ (num2 >> 13)) * 3770947547U;
                return num3 ^ (num3 >> 16);
            }
